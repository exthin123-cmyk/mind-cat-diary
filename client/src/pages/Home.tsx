...
// 일기 내용 기반 실시간 솔루션 생성 엔진
function generateDiarySolution(title: string, mood: string, thanks: string) {
  const text = `${title} ${mood} ${thanks}`.toLowerCase();
  
  const solutions = [
    {
      condition: () => text.includes("일") || text.includes("직장") || text.includes("업무") || text.includes("회사"),
      tip: "업무 스트레스는 잠깐의 환기로 해소할 수 있어요냥.",
      steps: ["5분간 자리에서 일어나 창문을 열고 바깥 공기를 마셔보기냥.", "오늘 완료한 업무 목록을 적어보며 성취감을 느껴보기냥."],
      music: "불안 😰"
    },
    {
      condition: () => text.includes("친구") || text.includes("사람") || text.includes("관계"),
      tip: "소중한 관계를 더 깊게 가꿔보세요냥.",
      steps: ["오늘 생각난 소중한 사람에게 짧은 안부 메시지를 보내보기냥.", "좋은 기억을 사진첩에서 꺼내 다시 한번 바라보기냥."],
      music: "기쁨 😊"
    },
    {
      condition: () => text.includes("힘들") || text.includes("지쳐") || text.includes("피곤") || text.includes("쉬고"),
      tip: "지금 당신에게 가장 필요한 건 완전한 휴식이에요냥.",
      steps: ["스마트폰을 내려놓고 15분간 눈을 감고 누워있기냥.", "좋아하는 따뜻한 음료를 마시며 아무 생각 없이 쉬어보기냥."],
      music: "피곤 😴"
    },
    {
      condition: () => text.includes("슬") || text.includes("울") || text.includes("눈물") || text.includes("아프"),
      tip: "슬픔을 느끼는 건 당신이 살아있다는 증거예요냥.",
      steps: ["슬픈 감정을 억누르지 말고 충분히 울어도 괜찮다냥.", "슬픔이 지나간 후 좋아하는 음악을 틀어보기냥."],
      music: "슬픔 😢"
    },
    {
      condition: () => text.includes("행복") || text.includes("좋은") || text.includes("기쁜") || text.includes("신나"),
      tip: "이 행복한 감정을 더 오래 간직해 보세요냥!",
      steps: ["오늘 기뻤던 순간을 한 줄 더 자세히 기록해보기냥.", "이 행복을 소중한 사람과 나눠보기냥."],
      music: "기쁨 😊"
    },
    {
      condition: () => text.includes("외로") || text.includes("혼자") || text.includes("쓸쓸"),
      tip: "외로움은 더 깊은 연결을 원하는 마음의 신호예요냥.",
      steps: ["커뮤니티 '마음 숲'에 오늘 기분을 나눠보기냥.", "좋아하는 반려동물 영상이나 편안한 음악을 틀어두기냥."],
      music: "외로움 🥺"
    }
  ];
  ...
  if (matched) {
    return { tip: matched.tip, steps: matched.steps, music: matched.music };
  }
  ...
  const defaults = [
    ...
    { tip: "당신의 감정은 모두 소중하고 유효해요냥.", steps: ["오늘 느낀 감정을 판단 없이 그대로 받아들여보기냥.", "좋아하는 향초나 아로마를 켜고 5분간 명상해보기냥."], music: "default" }
  ];
...
export default function Home() {
  ...
  const [isStoreOpen, setIsStoreOpen] = useState(false); // Stripe 결제 상점
  ...
  const [events, setEvents] = useState<ScheduleEvent[]>([
    ...
    { id: "e2", date: "2026-02-19", title: "프로젝트 마감일", mood: "기쁨 - 마침내 완성해서 신난다", thanks: "끝까지 포기하지 않은 내 자신에게 감사하다" }
  ]);
  ...
  const [feedPosts, setFeedPosts] = useState<FeedPost[]>([
    {
      id: "p1", author: "냥이집사3호", authorLevel: 5,
  ...
  useEffect(() => {
    ...
    if (params.get("payment") === "success") {
      ...
      if (productId === "apple_small") { setApples(a => a + 10); toast.success("사과 10개가 충전되었다냥! 🍎"); }
      else if (productId === "apple_medium") { setApples(a => a + 30); toast.success("사과 30개가 충전되었다냥! 🍎🍎🍎"); }
      else if (productId === "apple_large") { setApples(a => a + 100); toast.success("사과 100개가 충전되었다냥! 🎁"); }
      else if (productId === "premium_monthly") { setIsPremium(true); setApples(a => a + 50); toast.success("프리미엄 집사 멤버십이 활성화되었다냥! 👑"); }
  ...
  const toggleMusic = () => {
    ...
    if (isPlaying) {
      ...
      }).catch(() => toast.error("음악 재생에 실패했다냥!"));
    }
  };
  ...
  const handleAnswerSelect = (score: Partial<Record<MoodType, number>>) => {
    ...
    if (nextIndex < activeQuestions.length) {
      ...
      (Object.keys(testScores) as MoodType[]).forEach(key => {
        if (testScores[key] > maxScore) { maxScore = testScores[key]; selectedCat = key; }
      });
  ...
  const handleSendChat = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (lower.includes("안녕") || lower.includes("하이")) {
        catReply = `안녕 드림님! [${currentCatName}]이 반갑게 손을 흔든다냥! ☀️`;
      } else if (lower.includes("슬퍼") || lower.includes("우울") || lower.includes("힘들")) {
        catReply = `많이 힘들었겠다냥... 토닥토닥. [${currentCatName}]인 내가 드림님 곁에서 따뜻하게 안아줄 테니 걱정 말라냥 🐾❤️`;
      } else if (lower.includes("행복") || lower.includes("기뻐") || lower.includes("신나")) {
        catReply = `우와냥! 드림님이 행복하다니 나도 꼬리가 살랑살랑 춤을 춘다냥! 🍎⚡`;
      } else if (lower.includes("졸려") || lower.includes("피곤") || lower.includes("잘래")) {
        catReply = `오늘 정말 수고 많았다냥. 포근한 베개 꼭 베고 예쁜 꿈 꾸며 꿀잠 자라냥 😴💤`;
      } else if (lower.includes("불안") || lower.includes("걱정")) {
        catReply = `걱정하지 말라냥. 불안할 땐 나랑 같이 숨을 크게 들이마시고 내쉬어보자냥 🐾`;
      } else {
  ...
  // 일기 추가 (실시간 솔루션 + Lofi 추천)
  const handleAddEvent = (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputTitle.trim()) { toast.error("일기 제목을 입력해 달라냥!"); return; }
    const solution = generateDiarySolution(inputTitle, inputMood, inputThanks);
    const newEvent: ScheduleEvent = {
      ...
      mood: inputMood,
      thanks: inputThanks,
      customSolution: `${solution.tip} | ${solution.steps.join(" | ")}`,
      customMusicRecommendation: LOFI_PLAYLIST[solution.music]?.title || LOFI_PLAYLIST.default.title
    };
    ...
    setInputThanks("");
    gainExp(30, "마음 일기 작성");
    const musicToPlay = LOFI_PLAYLIST[solution.music] || LOFI_PLAYLIST.default;
    ...
    if (myItems.includes(item.id)) {
      if (equippedItems.includes(item.id)) {
        ...
        if (item.category === "wallpaper") {
          setEquippedItems(prev => [...prev.filter(id => SHOP_ITEMS.find(si => si.id === id)?.category !== "wallpaper"), item.id]);
  ...
  const handleStripeCheckout = async (productId: string) => {
    if (!isAuthenticated) {
      toast.error("결제를 진행하려면 로그인이 필요하다냥!");
    ...
    try {
      ...
      toast.error("결제 세션 생성에 실패했다냥. 잠시 후 다시 시도해달라냥!");
    }
  };

  // 커뮤니티
  const handleLikePost = (postId: string) => {
    setFeedPosts(prev => prev.map(post => post.id === postId ? { ...post, likes: post.likedByMe ? post.likes - 1 : post.likes + 1, likedByMe: !post.likedByMe } : post));
  };
  ...
  const handleCreatePost = (e: React.FormEvent) => {
    ...
    setFeedPosts([{
      id: Date.now().toString(), author: userName, authorLevel: level,
      avatar: CAT_CHARACTERS[catMood].image, content: `${newPostText}냥!`,
      likes: 0, likedByMe: false, comments: [], date: "방금 전",
      hasBestBadge: level >= 3
    }, ...feedPosts]);
    setNewPostText("");
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <button key={day} onClick={() => setSelectedDateStr(dateStr)}
          className={`h-10 w-full flex flex-col items-center justify-center rounded-lg transition-all relative ${isSelected ? "bg-blue-500 text-white font-bold" : "bg-white text-black hover:bg-gray-50"}`}>
      ...
      <div className="flex-1 flex flex-col bg-white h-full overflow-y-auto">
          ...
          <div className="grid grid-cols-4 gap-3">
            {(["unfair", "anxious", "lonely", "lethargic"] as MoodType[]).map(mood => (
              <div key={mood} className="flex flex-col items-center gap-1">
                ...
                <span className="text-[9px] font-bold text-gray-500 text-center leading-tight">{CAT_CHARACTERS[mood].name.split(" ")[0]}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="px-6 py-4 space-y-3">
          {[
            ...
            { icon: "📖", title: "일기 쓰면 맞춤 솔루션 제공", desc: "일기 내용을 분석해 실질적인 해결책과 Lofi 음악을 추천한다냥!" },
            { icon: "💬", title: "냥이와 대화하기", desc: "언제든지 감정냥이에게 속마음을 털어놓을 수 있다냥!" },
            { icon: "🌳", title: "마음 숲 커뮤니티", desc: "다른 집사들과 감정을 나누고 서로 위로해준다냥!" }
          ].map((item, idx) => (
            <div key={idx} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100">
              <span className="text-xl shrink-0">{item.icon}</span>
              <div>
                <h3 className="text-xs font-black text-gray-800">{item.title}</h3>
        ...
        <div className="px-6 py-6 space-y-3">
          <button
            ...
            className="w-full py-4 bg-blue-500 hover:bg-blue-600 text-white font-bold text-sm rounded-2xl transition-colors shadow-md flex items-center justify-center gap-2"
          >
            <UserPlus className="w-4 h-4" /> 무료로 시작하기냥 🐾
          </button>
          <button
            onClick={() => window.location.href = getLoginUrl()}
            className="w-full py-3 bg-white hover:bg-gray-50 text-gray-700 font-bold text-sm rounded-2xl border border-gray-200 transition-colors flex items-center justify-center gap-2"
          >
            <LogIn className="w-4 h-4" /> 이미 회원이라면 로그인
          </button>
          <button
            onClick={() => { setIsTestCompleted(false); setAuthView("app"); }}
            className="w-full py-3 bg-gray-50 hover:bg-gray-100 text-gray-500 font-bold text-xs rounded-2xl border border-gray-100 transition-colors"
          >
            로그인 없이 체험해보기
          </button>
        </div>
  ...
  if (authView === "signup") {
    return (
      <div className="flex-1 flex flex-col bg-white h-full overflow-y-auto p-6">
        <button onClick={() => setAuthView("landing")} className="flex items-center gap-1 text-gray-400 text-xs font-bold mb-6">
          <ChevronLeft className="w-4 h-4" /> 돌아가기
        </button>

        <div className="text-center space-y-2 mb-8">
          <div className="text-4xl">🐾</div>
          <h2 className="text-xl font-black text-gray-800">나만의 감정냥이 만들기</h2>
          <p className="text-xs text-gray-500 font-bold">닉네임과 고양이 이름을 설정하고 심리테스트를 시작하세요냥!</p>
        </div>

        <div className="space-y-5 flex-1">
          <div className="space-y-1.5">
            <label className="text-xs font-black text-gray-600">나의 닉네임</label>
            <input
              type="text"
              value={signupNickname}
              onChange={(e) => setSignupNickname(e.target.value)}
              placeholder="예: 드림님, 집사, 냥냥이..."
              className="w-full px-4 py-3 rounded-xl border border-gray-200 font-bold text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>

          <div className="space-y-1.5">
            <label className="text-xs font-black text-gray-600">나의 감정냥이 이름</label>
            <input
              type="text"
              value={signupCatName}
              onChange={(e) => setSignupCatName(e.target.value)}
              placeholder="예: 드림이, 뭉치, 나비..."
              className="w-full px-4 py-3 rounded-xl border border-gray-200 font-bold text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>

          <div className="p-4 rounded-2xl bg-blue-50 border border-blue-100 space-y-2">
            <h3 className="text-xs font-black text-blue-700">📋 회원가입 혜택</h3>
            <ul className="text-[11px] text-blue-600 font-bold space-y-1">
              <li>✅ 심리테스트로 나만의 감정냥이 매칭</li>
              <li>✅ 일기 작성 시 맞춤 솔루션 & Lofi 음악 추천</li>
              <li>✅ 냥이와 대화하기 무제한 이용</li>
              <li>✅ 마음 숲 커뮤니티 참여</li>
              <li>✅ 사과 5개 가입 축하 선물 🍎</li>
            </ul>
          </div>
        </div>
        ...
        <button
          onClick={() => {
            ...
            if (!signupCatName.trim()) { toast.error("고양이 이름을 입력해달라냥!"); return; }
            setUserName(signupNickname);
            setCatName(signupCatName);
            setApples(5); // 가입 축하 사과 5개
            setAuthView("app");
            setIsTestCompleted(false);
            toast.success(`환영한다냥, ${signupNickname}님! 사과 🍎 5개를 선물로 드린다냥!`);
          }}
          className="w-full py-4 bg-blue-500 hover:bg-blue-600 text-white font-bold text-sm rounded-2xl transition-colors shadow-md mt-6"
        >
          가입하고 심리테스트 시작하기냥 🐾
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between p-6 bg-white h-full overflow-y-auto">
        <div className="text-center space-y-2 mt-6">
          ...
          <p className="text-xs text-gray-500 font-medium">
            검사할 때마다 새로운 질문지로 매번 다른 결과를 받아볼 수 있다냥!
          </p>
        </div>
        ...
        <div className="my-8 p-6 rounded-2xl bg-gray-50 border border-gray-100 text-center space-y-6 shadow-sm">
          ...
          <div className="space-y-3">
            {q.options.map((opt, idx) => (
              <button key={idx} onClick={() => handleAnswerSelect(opt.score)}
        ...
        <div className="space-y-2 mb-6">
          <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${((currentQuestionIndex + 1) / 10) * 100}%` }}></div>
          </div>
  ...
  return (
    <div className="flex-1 flex flex-col relative h-full bg-white font-sans overflow-hidden">
      ...
      <header className="px-5 py-3.5 flex items-center justify-between border-b border-gray-100 bg-white z-10">
        <button onClick={() => setIsMailOpen(true)} className="p-2 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors border border-gray-100">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setIsSettingsOpen(true)} className="p-2 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors border border-gray-100">
      ...
      <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <div className="bg-gray-800 text-white text-[10px] font-black px-2 py-0.5 rounded-full flex items-center gap-1">
            {isPremium && <Crown className="w-2.5 h-2.5 text-yellow-300" />}
            Lv.{level}
          </div>
          <div className="w-28 h-2 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${(exp / maxExp) * 100}%` }}></div>
          </div>
        ...
        {activeTab === "room" && (
          <div className="p-5 space-y-5 h-full flex flex-col justify-between min-h-[460px]">
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={() => setIsShopOpen(true)} className="flex items-center gap-1.5 px-5 py-3 bg-gray-800 text-white font-bold text-xs rounded-xl shadow-sm hover:bg-gray-700 transition-colors">
                <ShoppingBag className="w-4 h-4" /> 소품 상점
              </button>
              <button onClick={() => { setActiveTab("chat"); setBubbleText("무슨 재밌는 얘기를 들려줄 거냐냥? 🐾"); }}
                ...
                <MessageSquare className="w-4 h-4" /> 냥이와 대화하기
              </button>
            </div>
          </div>
        )}
        ...
        {activeTab === "chat" && (
          <div className="absolute inset-0 bottom-20 bg-white flex flex-col z-20">
            <div className="px-5 py-3 bg-blue-50 border-b border-blue-100 flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setActiveTab("room")} className="text-xs text-gray-500 hover:text-gray-700 font-bold bg-white px-2.5 py-1 rounded-lg border border-gray-150">
                방으로 가기
              </button>
            </div>
            ...
            <form onSubmit={handleSendChat} className="p-3 border-t border-gray-100 bg-white flex gap-2 shrink-0">
              ...
              <button type="submit" className="p-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition-colors shadow-sm shrink-0">
        ...
        {activeTab === "calendar" && (
          <div className="p-5 space-y-4">
            <div className="flex items-center justify-between">
              <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1))} className="p-2 rounded-lg bg-gray-50 border border-gray-100">
            ...
            <div className="space-y-3 mt-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setIsAddEventOpen(true)} className="flex items-center gap-1 px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white font-bold text-xs rounded-lg transition-colors shadow-sm">
                  <Plus className="w-3 h-3" /> 일기 쓰기
                </button>
              </div>
                ...
                events.filter(e => e.date === selectedDateStr).map(event => {
                  ...
                  return (
                    <div key={event.id} className="border border-gray-100 rounded-2xl p-5 bg-white space-y-4 shadow-sm relative">
                      <button onClick={() => { setEvents(prev => prev.filter(e => e.id !== event.id)); toast.success("일기를 삭제했다냥."); }}
                      ...
                      <div className="bg-blue-50/50 rounded-xl p-4 border border-blue-50/80 space-y-2.5">
                        ...
                        <p className="text-[10px] text-blue-600 font-bold">{solution.tip}</p>
        ...
        {activeTab === "community" && (
          <div className="p-5 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setIsCommunityWriteOpen(true)} className="flex items-center gap-1 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white font-bold text-xs rounded-xl transition-colors shadow-sm">
              ...
              {feedPosts.map(post => (
                <div key={post.id} className="border border-gray-100 rounded-2xl bg-white shadow-sm overflow-hidden">
                  ...
                  <div className="p-4 border-b border-gray-50">
                    <p className="text-xs font-bold text-gray-700 leading-relaxed">{post.content}</p>
                  </div>
                  <div className="px-4 py-2.5 bg-gray-50/20 flex items-center gap-4 border-b border-gray-50">
                    <button onClick={() => handleLikePost(post.id)} className={`flex items-center gap-1 text-xs font-bold transition-colors ${post.likedByMe ? "text-red-500" : "text-gray-500 hover:text-red-500"}`}>
                      <Heart className={`w-3.5 h-3.5 ${post.likedByMe ? "fill-current" : ""}`} /><span>{post.likes}</span>
                    </button>
                    <div className="flex items-center gap-1 text-xs font-bold text-gray-500">
                      <MessageCircle className="w-3.5 h-3.5" /><span>{post.comments.length}</span>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-white flex gap-2">
                    <input type="text" placeholder="댓글을 달아달라냥..."
                      onKeyDown={(e) => { if (e.key === 'Enter') { handleAddComment(post.id, (e.target as HTMLInputElement).value); (e.target as HTMLInputElement).value = ""; } }}
                      className="flex-1 px-3 py-2 rounded-lg border border-gray-150 bg-white font-bold text-xs focus:outline-none" />
                    <button onClick={(e) => { const input = (e.currentTarget.previousSibling as HTMLInputElement); handleAddComment(post.id, input.value); input.value = ""; }}
                      className="px-3 bg-gray-800 text-white text-xs font-bold rounded-lg">등록</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="p-5 space-y-5">
            ...
            {!isAdminLoggedIn ? (
              <div className="border border-gray-100 rounded-2xl p-5 bg-white shadow-sm space-y-4">
                ...
                <button onClick={() => {
                  if (adminPassword === "1234" || adminPassword === "admin") { setIsAdminLoggedIn(true); toast.success("관리자 권한으로 로그인했다냥! 🛡️"); }
                  else toast.error("비밀번호가 틀렸다냥!");
                }} className="w-full py-2.5 bg-gray-800 text-white font-bold text-xs rounded-xl">로그인</button>
              </div>
            ) : (
              <div className="space-y-5">
                ...
                <div className="border border-gray-100 rounded-2xl p-5 bg-white shadow-sm space-y-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { name: "드림님", level: 3, cat: "억울냥", joinDate: "2026-05-01", premium: true },
                      { name: "냥이집사3호", level: 5, cat: "사랑냥", joinDate: "2026-04-15", premium: false },
                      { name: "행복한하루", level: 2, cat: "외롭냥", joinDate: "2026-05-20", premium: false }
                    ].map((member, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-100">
                        <div>
                          <div className="flex items-center gap-1.5">
                            ...
                            {member.premium && <span className="text-[8px] bg-amber-500 text-white px-1 py-0.2 rounded font-bold">👑</span>}
                          </div>
                ...
                <div className="border border-gray-100 rounded-2xl p-5 bg-white shadow-sm space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1">
                      ...
                      <input type="text" value={adText} onChange={(e) => setAdText(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-gray-200 font-bold text-xs" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400">광고 아웃링크 URL</label>
                      <input type="text" value={adLink} onChange={(e) => setAdLink(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-gray-200 font-bold text-xs" />
                    </div>
                ...
                <div className="border border-gray-100 rounded-2xl p-5 bg-white shadow-sm space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={() => { setEvents([]); toast.success("모든 일정이 초기화되었다냥."); }} className="py-2 bg-red-50 hover:bg-red-100 text-red-600 font-bold text-xs rounded-lg">🧹 일정 초기화</button>
      ...
      <div className="absolute bottom-20 left-0 right-0 h-12 bg-gray-50 border-t border-b border-gray-100 px-5 flex items-center justify-between z-10">
        ...
        <a href={adLink} target="_blank" rel="noreferrer" className="text-[9px] text-blue-500 font-bold flex items-center gap-0.5 hover:underline shrink-0">
      ...
      <nav className="absolute bottom-0 left-0 right-0 h-20 bg-white border-t border-gray-100 flex items-center justify-around px-4 z-10 shadow-lg">
        <button onClick={() => setActiveTab("chat")} className={`flex flex-col items-center justify-center w-12 h-12 rounded-xl transition-all ${activeTab === "chat" ? "text-blue-500 scale-105" : "text-gray-400"}`}>
          <MessageSquare className="w-5 h-5" />
        </button>
        <button onClick={() => setActiveTab("calendar")} className={`flex flex-col items-center justify-center w-12 h-12 rounded-xl transition-all ${activeTab === "calendar" ? "text-blue-500 scale-105" : "text-gray-400"}`}>
          <CalendarIcon className="w-5 h-5" />
        </button>
        <button onClick={() => setActiveTab("room")} className={`flex flex-col items-center justify-center w-14 h-14 rounded-2xl -translate-y-4 transition-all shadow-md ${activeTab === "room" ? "bg-blue-500 text-white" : "bg-gray-50 text-gray-400"}`}>
          <img src={CAT_CHARACTERS[catMood].image} alt="Cat Icon" className="w-9 h-9 object-contain" />
        </button>
        <button onClick={() => setActiveTab("community")} className={`flex flex-col items-center justify-center w-12 h-12 rounded-xl transition-all ${activeTab === "community" ? "text-blue-500 scale-105" : "text-gray-400"}`}>
          <Users className="w-5 h-5" />
        </button>
        <button onClick={() => setActiveTab("admin")} className={`flex flex-col items-center justify-center w-12 h-12 rounded-xl transition-all ${activeTab === "admin" ? "text-blue-500 scale-105" : "text-gray-400"}`}>
        ...
        <div className="absolute inset-0 bg-black/30 z-50 flex items-end md:items-center justify-center p-4">
          <div className="w-full max-w-[380px] bg-white rounded-3xl p-5 space-y-4 animate-in slide-in-from-bottom duration-200 border border-gray-100 shadow-xl">
            <div className="flex items-center justify-between border-b border-gray-50 pb-2">
              <div>
                <h3 className="text-sm font-bold text-gray-800">사과 충전 & 프리미엄 상점</h3>
                <p className="text-[10px] text-gray-400 font-bold">카드 번호 4242 4242 4242 4242로 테스트 결제 가능</p>
              </div>
              ...
              {[
                ...
                <div key={product.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-xs text-gray-700">{product.name}</h4>
                      <p className="text-[9px] text-gray-400 font-bold">{product.desc}</p>
                      <p className="text-[10px] text-blue-600 font-black mt-0.5">🍎 +{product.apples}개</p>
                    </div>
                  </div>
                  <button onClick={() => handleStripeCheckout(product.id)}
                    disabled={createCheckout.isPending}
                    className="px-3 py-1.5 rounded-lg text-xs font-bold bg-blue-500 hover:bg-blue-600 text-white shadow-sm transition-all disabled:opacity-50 shrink-0">
                    {createCheckout.isPending ? "..." : product.price}
                  </button>
                </div>
              ))}
            </div>
            ...
            <p className="text-[9px] text-gray-400 text-center font-bold">
              테스트 카드: 4242 4242 4242 4242 | 유효기간: 미래 임의 날짜 | CVV: 임의 3자리
            </p>
          </div>
        </div>
      )}
      ...
      {isShopOpen && (
        <div className="absolute inset-0 bg-black/30 z-50 flex items-end md:items-center justify-center p-4">
          <div className="w-full max-w-[380px] bg-white rounded-3xl p-5 space-y-4 animate-in slide-in-from-bottom duration-200 border border-gray-100 shadow-xl">
            <div className="flex items-center justify-between border-b border-gray-50 pb-2">
              <div className="flex items-center gap-1.5">
                <h3 className="text-sm font-bold text-gray-800">꾸미기 소품 상점</h3>
                <span className="text-[10px] bg-gray-50 border border-gray-100 text-gray-600 px-2 py-0.5 rounded-full flex items-center gap-1 font-bold">🍎 {apples}개</span>
              </div>
                ...
                return (
                  <div key={item.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50/50">
                    ...
                    <button onClick={() => handleBuyItem(item)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${isEquipped ? "bg-gray-800 text-white" : isBought ? "bg-gray-200 text-gray-700" : "bg-blue-500 hover:bg-blue-600 text-white shadow-sm"}`}>
            ...
            <button onClick={() => { setIsShopOpen(false); setIsStoreOpen(true); }} className="w-full py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 font-bold text-xs rounded-lg flex items-center justify-center gap-1">
              <CreditCard className="w-3.5 h-3.5" /> 사과가 부족하면? 충전하러 가기냥 →
            </button>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-[340px] bg-white rounded-3xl p-6 space-y-4 border border-gray-100 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setIsMailOpen(false)} className="p-1 rounded-lg hover:bg-gray-50"><X className="w-4 h-4 text-gray-400" /></button>
            </div>
        ...
        <div className="absolute inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-[340px] bg-white rounded-3xl p-6 space-y-4 border border-gray-100 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setIsSettingsOpen(false)} className="p-1 rounded-lg hover:bg-gray-50"><X className="w-4 h-4" /></button>
            </div>
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400">고양이 이름</label>
                <input type="text" value={catName} onChange={(e) => setCatName(e.target.value)} className="w-full px-3.5 py-2 rounded-xl border border-gray-200 font-bold text-xs" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400">사용자 닉네임</label>
                <input type="text" value={userName} onChange={(e) => setUserName(e.target.value)} className="w-full px-3.5 py-2 rounded-xl border border-gray-200 font-bold text-xs" />
              </div>
              <button onClick={() => { setIsSettingsOpen(false); toast.success("설정이 저장되었습니다냥! 🐾"); }} className="w-full py-2.5 bg-gray-800 hover:bg-gray-700 text-white font-bold text-xs rounded-xl">설정 완료</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-[340px] bg-white rounded-3xl p-5 space-y-4 border border-gray-100 shadow-xl">
            <div className="flex items-center justify-between border-b border-gray-50 pb-2">
              ...
              <button onClick={() => setIsAddEventOpen(false)} className="p-1 rounded-lg hover:bg-gray-50"><X className="w-4 h-4 text-gray-400" /></button>
            </div>
            <form onSubmit={handleAddEvent} className="space-y-3.5">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400">날짜</label>
                <input type="text" value={selectedDateStr} disabled className="w-full px-3 py-2 bg-gray-50 rounded-xl border border-gray-100 font-bold text-xs text-gray-500" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400">일기 제목</label>
                <input type="text" value={inputTitle} onChange={(e) => setInputTitle(e.target.value)} placeholder="예: 오늘 중요한 발표를 끝냈다!" className="w-full px-3 py-2 rounded-xl border border-gray-200 font-bold text-xs focus:outline-none focus:ring-1 focus:ring-blue-500" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400">오늘의 핵심 감정</label>
                <select value={inputMood} onChange={(e) => setInputMood(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-gray-200 font-bold text-xs focus:outline-none">
                  <option>기쁨 😊</option>
                  <option>슬픔 😢</option>
                  <option>피곤 😴</option>
                  <option>불안 😰</option>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400">오늘 하루 감사했던 일</label>
                <textarea value={inputThanks} onChange={(e) => setInputThanks(e.target.value)} placeholder="아무리 작은 일이라도 감사함을 느껴보라냥..." rows={2} className="w-full px-3 py-2 rounded-xl border border-gray-200 font-bold text-xs focus:outline-none resize-none" />
              </div>
              <button type="submit" className="w-full py-2.5 bg-blue-500 hover:bg-blue-600 text-white font-bold text-xs rounded-xl transition-colors shadow-sm">
                일기 쓰고 맞춤 해결책 & 음악 받기냥 🐾
              </button>
            </form>
          </div>
        ...
        <div className="absolute inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-[340px] bg-white rounded-3xl p-5 space-y-4 border border-gray-100 shadow-xl">
            <div className="flex items-center justify-between border-b border-gray-50 pb-2">
              ...
              <button onClick={() => setIsCommunityWriteOpen(false)} className="p-1 rounded-lg hover:bg-gray-50"><X className="w-4 h-4 text-gray-400" /></button>
            </div>
